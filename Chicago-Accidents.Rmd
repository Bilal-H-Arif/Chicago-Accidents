...
```{r}
date <- today() - years(3)
crash_url <- glue::glue("https://data.cityofchicago.org/Transportation/Traffic-Crashes-Crashes/85ca-t3if?$where=CRASH_DATE > '{date}'")
...
```{r How have the number of crashes changed over time?}
crash %>%
...
```{r How Has the Injury Rate Changed over Time}
crash %>%
  mutate(crash_date = floor_date(crash_date, unit = "week")) %>%
  count(crash_date, injuries) %>%
  filter(
